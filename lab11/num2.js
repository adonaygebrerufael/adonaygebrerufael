...
const server = http.createServer((req, res) => {
  ...
  if (req.url === '/image') {
    ...
    fs.createReadStream('./bestpicture.jpeg').pipe(res); // Replace with the actual image path
  } else if (req.url === '/pdf') {
    // Serve a PDF file
    res.setHeader('Content-Type', 'application/pdf'); // Set the content type for PDF
    fs.createReadStream('./lab6.pdf').pipe(res); // Replace with the actual PDF path
